...
/**
 * Characters are 8x5 pixels
 */
#define CHAR_HEIGHT	8
#define CHAR_WIDTH	5

/**
 * ASCII table (begin at 32; extended character not included)
 */
const char font[96][CHAR_WIDTH] = {
...
/**
 * Put text into screen according position (x,y)
 */
void tput(int x, int y, const char* str) {
	//Permit to get text on multiple lines
	unsigned char newLine = 1;
	//Count number of character (useful to avoiding character juxtaposition) 
	for (int charCount = 0; charCount < strlen(str); charCount++) {
		//if next character get out of screen
		if ((charCount+1)*(CHAR_WIDTH+1) > (newLine*WIN_WIDTH - x)) {
			y += CHAR_HEIGHT;
			x += x/newLine;
			newLine++;
		}
		//Count x value of current pixel
		for (int width = 0; width < CHAR_WIDTH; width++) {
			unsigned char charPixel = font[(int)str[charCount]-32][width];
			//Count y value of current pixel
			for (int height = 0; height <= CHAR_HEIGHT; height++) {
				if (charPixel & 1) Screen[(CHAR_WIDTH+1)*charCount+(width+x)+(height+y)*240] = 0x7FFF;
				charPixel >>= 1;
			}
		}
	}
